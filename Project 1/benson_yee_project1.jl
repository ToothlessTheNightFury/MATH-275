...
# ╔═╡ 9a353dfd-84eb-471b-b82b-f86a45416148
# ╠═╡ show_logs = false
begin
	using DifferentialEquations, LaTeXStrings, Plots, PlutoUI; plotly()
	theme(:dracula)  # color theme for plots
end

# ╔═╡ 0ff5ef43-155d-4106-af81-a02bb23da381
md"""
...
# ╔═╡ 72246c9b-6d07-42c0-9922-e15f37a4716f
md"""
in which all terms have units of kilograms per second. At $t = 0$ the tank contains $0$ kg of salt, so the initial condition is $x(0) = 0$. The above equation is a linear first-order differential equation.
"""

# ╔═╡ a4343345-fbac-4763-abc2-d104a4f8652a
md"""
#### Answer

An appropiate ODE for $x(t)$ with the given conditions is
"""

# ╔═╡ dc49f05d-e54c-40f2-af2d-0686ff2d1c19
md"""
## (b) Solve ODE using analyical solution (integrating factor)
The equation for $x(t)$ is a linear first-order differential equation. Solving this equation using the integrating factor approach yields
"""

# ╔═╡ 9126d014-8324-48ba-876b-883744463da9
md"""
#### Answer
...
# ╔═╡ d372af6d-a3d1-439f-98b6-e02b7c5bb9c4
md"""
## (c) Solve ODE using numerical solution (DifferentialEquations.jl)
"""

# ╔═╡ 1de62b24-2e44-456b-b11b-37dbd5e04048
md"""
We will solve the ODE
"""

# ╔═╡ 0edf90c3-d3d5-4142-95d8-07acaef71a9d
md"""
using `DifferentialEquations.jl`. To do so, we will perform the following steps:
"""

# ╔═╡ 65e10ce7-cda4-4157-ba4c-9ffc9b1ade92
md"""
##### 1. Define Julia function representing the ODE

```
function ode(u, p, t)
	y = u  # to represent the ODE in terms of the original variable y.
	vol, con, r = p # volume, concentration, rate, stored in parameter
	dy = (r * con) - y / (vol / r)
end
```
"""

# ╔═╡ 829d13cf-d444-492a-800c-aad74a655467
function ode(u, p, t)
	y = u  # to represent the ODE in terms of the original variable y.
	vol, con, r = p # volume, concentration, rate, stored in parameter
	dy = (r * con) - y / (vol / r)
...
# ╔═╡ 3256c0c9-8e45-468e-97b6-319528f11f69
md"""
##### 2. Define an initial condition $x(0)$.
...
# ╔═╡ c91b9107-2df8-4e25-a331-b761fdfe79da
x01 = 0;  # This is x(0)

# ╔═╡ 7d9e33a5-0a7c-4c3d-bc7d-60ee11955d5a
md"""
##### 3. Define the interval *tspan* over which we are solving the ODE.
```
tspan = (0, 600)
```
"""

# ╔═╡ 982192e3-3399-4a99-b949-0d21f361c2f6
tspan = (0, 600);

# ╔═╡ 49436a7c-01cf-4274-859b-d3bc9a205edb
md"""
##### 4. Define the problem using the *ODEProblem* function

```
prob1 = ODEProblem(ode, x01, tspan, p)
```
"""

# ╔═╡ f59b85ce-1471-4488-9139-67fb85e7a112
md"""
##### 5. Solve the problem using the *solve* function

```
numerical_sol = solve(prob1)
```
"""

# ╔═╡ de835dd0-a21c-40ec-9259-c04d5ff1ef7b
md"""
## (d) Plot analyical and numerical solution
"""

# ╔═╡ d43ab771-4f73-49e9-81ca-987915d3a533
begin
	volume = 400
	concentration = 0.2
	rate = 4.0
	...
	Use the sliders below to modify the problem.
	
	**Volume:** $(@bind volume Slider(100:10:800, volume, true)) liters
	
	**Concentration of Salt:** $(@bind concentration Slider(0:0.01:0.5, concentration, true)) kg salt per liter
	
	**Rate of Flow:** $(@bind rate Slider(1:0.1:8, rate, true)) liters per minute
	"""
end

# ╔═╡ c54b1760-a28b-11ed-0171-e70d73a54bf5
md"""
# Project 1: Salt Tank Problem

A tank contains $volume liters of pure water. At time t = 0 water containing $concentration kg of salt per liter begins to flow into the tank at a rate of $rate liters per minute. The well-stirred mixture flows out of the tank at $rate liters per minute. 

**(a) Formulate an appropriate ODE for $x(t)$, the amount of salt in the tank at time $t$, with an initial condition.**

**(b) Solve the ODE using analyical solution via integrating factor.**

**(c) Solve the ODE using numerical solution via DifferentialEquations.jl.**

**(d) Plot analyical and numerical solution.**
"""

# ╔═╡ c433a50d-fe61-4840-ad7f-a6f109f81a15
md"""
## (a) Formulate ODE for $x(t)$
**Based on 2.1 First-Order Linear Equations, pg. 31**

To find the rate at which salt enters the tank at the inflow pipe, note that $rate liters of fluid enters each minute and each liter contains $concentration kg of salt. The rate at which salt enters is thus
"""

# ╔═╡ 85410fa2-3edb-4a4f-ab04-2bd1ab54ae4e
md"""
The rate at which salt is leaving the tank can be found by noting that since the tank is well-stirred, the concentration of salt in the tank is spatially uniform, and so each liter of tank fluid at time $t$ contains $x(t)$ = $volume kg of salt. Precisely $rate liters of this solution exit the tank each minute, thus the rate at which salt is leaving the tank is 
"""

# ╔═╡ 84a69e02-bd97-4356-b99f-c8d8dc366f50
begin
	rateOfSaltEnter = rate * concentration
	rateOfSaltExit = volume / rate
	solutionConst = rateOfSaltEnter * rateOfSaltExit
end;

# ╔═╡ 9cc68f54-a85c-4ea5-ad77-6633b2e6633a
L"""
%$(rate) \frac{\text{liters}}{\text{minute}} \times %$(concentration) \frac{\text{kg}}{\text{liter}} = %$(rateOfSaltEnter) \frac{\text{kg}}{\text{minute}}
"""

# ╔═╡ 807f3a44-506e-4754-9ecf-8c686e2eef6e
L"""
%$(rate) \frac{\text{liters}}{\text{minute}} \times \frac{x(t)}{%$(volume)} \times \frac{\text{kg}}{\text{liter}} = \frac{x(t)}{%$(rateOfSaltExit)} \frac{\text{kg}}{\text{minute}}
"""

# ╔═╡ 2d12ba88-090a-4bf4-93d1-99da1f1197d4
L"""
\frac{dx}{dt} = %$(rateOfSaltEnter) - \frac{x(t)}{%$(rateOfSaltExit)} \frac{\text{kg}}{\text{minute}}
"""

# ╔═╡ bd421693-f618-4904-b892-213079b9e652
L"""
\frac{dx}{dt} = %$(rateOfSaltEnter) - \frac{x(t)}{%$(rateOfSaltExit)}
"""

# ╔═╡ 1d3d7bf6-a69a-4d39-a300-5c8e78c97224
L"""
x(t) = %$(solutionConst) - %$(solutionConst)e^\frac{-t}{%$(rateOfSaltExit)}$
"""

# ╔═╡ 39519a37-e269-44f7-be36-cd85bd0c51c6
L"""
x(t) = %$(solutionConst) - %$(solutionConst)e^\frac{-t}{%$(rateOfSaltExit)}$
"""

# ╔═╡ d245e300-28a0-41cf-8600-a32b70c49366
analyical_sol(t) = solutionConst - solutionConst * exp(-t / rateOfSaltExit);

# ╔═╡ 377ffa2c-3151-4867-a4fb-8314594708c1
L"""
\frac{dx}{dt} = %$(rateOfSaltEnter) - \frac{x(t)}{%$(rateOfSaltExit)}
"""

# ╔═╡ 86e43b8a-0868-4c40-94b1-375cefd2ce3a
p = [volume, concentration, rate];

# ╔═╡ 827f91de-3dac-4933-9a81-0f61fd5f85b9
prob1 = ODEProblem(ode, x01, tspan, p);

# ╔═╡ 83b00b45-f22f-42d2-90ae-d6e1e94ab7b4
numerical_sol = solve(prob1)

# ╔═╡ b896a462-c79e-4d0e-bea4-ed3a0f878d88
begin
	plot(numerical_sol,
		title = "Salt Tank",
		xaxis = "Time (minute)",
		yaxis = "Salt in Tank (kg)",
		color = :darkviolet,
		linewidth = 4,
		xrange = (0, 650),
		label = "analyical model",
	);  # the ";" at the end of a Julia function suppresses display
