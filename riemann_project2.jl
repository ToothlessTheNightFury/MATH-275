...
# ╔═╡ c4664b10-afd0-11ed-0c0b-4322248e5623
md"""
# Project 2: Feral Cat Control
Feral cat colonies are a serious problem in many towns. **Alley Cat Advocates** has been working to control the growth of these populations for many years.

They have recently been approached by **Mr. Tom Kat**, a representative from a town with a very large feral cat colony. Mr. Kat is willing to fund a project to implement humane methods for population control in his town. **Alley Cat Advocates** will also have the opportunity to give public presentations nationwide on the proper protocol for controlling feral cat populations.

Mr. Kat has asked **Alley Cat Advocates** to produce a formal report utilizing simulated mathematical data to illustrate the efficacy of their suggestions. This report, developed by expert mathematical consultants from the **Riemann Group**, is produced on behalf of **Alley Cat Advocates**.

There are two commonly used methods for controlling feral cat populations:

* In the first method, traps are set for feral cats. When a cat is trapped, it is immediately euthanized. This is called the **"Trap-Kill"** method. 

* In the second method, traps are set, but when a cat is trapped, it is neutered / spayed and returned to the population. This is called the **"Trap-Neuter-Return"** method.

This report contains mathematical models and analysis of each of these methods, as well as recommendations for **Alley Cat Advocates** and Mr. Kat. **Definitions and explanations have been provided so it is readable for all.**
"""
...
The **Riemann Group** seeks to achieve the following goals in this report:
...
* (c) The critical number of traps for each method which determines if the population grows or declines
* (d) A comparison of the efficiency of **Trap-Kill** and **Trap-Neuter-Return**
"""

# ╔═╡ 6f443ded-d71b-408c-821b-312d71132d42
md"""
## Givens and Context
The following variables are used to model these situations:

$\begin{gather}
t = \text{time} \\
y(t) = \text{total population after time } t
\end{gather}$

The following parameters are used to model these situations. These parameters may be adjusted to model different situations:

$\begin{gather}
k = \text{\% rate of cat population growth} \\
y_0 = \text{starting population} \\
n = \text{the number of traps} \\
m = \text{average number of cats captured per year} \\
\end{gather}$

**Alley Cat Advocates** has provided research (see **[1]** and **[2]**) that shows the per-year per-cat growth-rate for most feral cat population is $k = 1.08$. This means, for most feral cat populations, the population is multiplied by $1.08$ every year (or increases by $8\%$).

Mr. Kat estimates the current feral cat population in his town is $P_0 = 100$.
...
* **[1]** Schmidt, P., T. Swannack, R. Lopez, and M. Slater. 2009. Evaluation of euthanasia and trapneutering (TNR) programs in managing free-roaming cat populations. *Wildlife Research.* 36:117-125.
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b76fba9897f243435eb24277f21c2ccbae83e87a"
...
git-tree-sha1 = "208146abb8f1766cba87ed986ff205428c2df040"
...
deps = ["ArrayInterfaceCore", "DiffEqBase", "FiniteDiff", "ForwardDiff", "LinearAlgebra", "Reexport", "Requires", "SciMLBase", "SnoopPrecompile", "StaticArraysCore"]
git-tree-sha1 = "8d5e6cf29233456d844d489a9e07b171ed124d6d"
